...
public class FragmentMystery extends Fragment {

    View v;
    RecyclerView rvMystery;
    BooksAdapter adapter;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        v = inflater.inflate(R.layout.mystery_fragment, container, false);
        rvMystery = (RecyclerView) v.findViewById(R.id.rvMystery);

        adapter = new BooksAdapter(getContext());
        rvMystery.setLayoutManager(new LinearLayoutManager(getActivity()));
        ...
        call.enqueue(new Callback<BooksResponse>() {
            ...
            public void onResponse(Call<BooksResponse> call, Response<BooksResponse> response) {
                for(int i = 0; i < response.body().products.size(); i++) {
                    if(response.body().products.get(i).category.equals("mystery")) {
